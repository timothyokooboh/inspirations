...
@section('content')
  ...
  <div class="main-grid-container">
    ...
    <div class="post-container">
      ...
      <form action="{{route('posts.patch', ['id' => $post->id])}}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PATCH')

        <div>
          <label for="title"> Post Title </label> <br>
          <input type="text" id="title" class="input" name="title" value = "{{$post->title}}">
        </div>
        ...
        <div>
          ...
          <textarea id="story" name="story"> {{$post->story}} </textarea>
        </div>

        <div>
          <label for="tags"> Tags (optional) </label>
          <p> Separate tags with comma </p>
          <input type="text" id="tags" class="input" name="tags" placeholder="e.g. health, relationships, career" value = "{{$post->tags}}" >
        </div>
